...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./new/shrip.json', 'utf8'));
    ...
    const data = jsonArray
      ...
      .map(product => {
        ...
        const mappedProduct = {
          ...
          sku: product?.sku,
          gtin: product?.gtin,
          image:"https://lombardis.storebyweb.com"+product?.image,
          pack: product?.pack,
          size: product?.size,
          ...
          distributor_name: product?.distributor || "Avalon Wine",
