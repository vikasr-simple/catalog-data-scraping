...
console.log(`Total SKUs to fetch: ${arr.length}`);
...
const fetchProducts = async (sku) => {
  const API_URL = `https://www.chefswarehouse.com/products/${sku}/?expand=*&currentPageUrl=%252Fproducts%252FDM116%252F&tz=Asia%252FCalcutta&t=1725005751994`;
...
const fetchAllProducts = async () => {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const promises = batch.map((product) => {
      const { sku } = product;
      if (!sku) {
        console.warn('SKU missing, skipping...');
        return Promise.resolve(null);
      }
      return fetchProducts(sku);
    });

    const results = await Promise.all(promises);
    allData.push(...results.filter(Boolean)); // Exclude null results
...
fetchAllProducts()
  .then(async (allData) => {
    ...
    const jsonData = JSON.stringify(allData, null, 2);
    await fs.writeFile('dairyProductData.json', jsonData, 'utf8');
    console.log('Data successfully saved to dairyProductData.json');
  })
  .catch((error) => {
    console.error('Error during fetching all products:', error.message);
  });
