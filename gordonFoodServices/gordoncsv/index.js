...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./json/SeaFood8_Data.json', 'utf8'));
    ...
    const data = jsonArray
      ...
      .map(product => {
        ...
        return {
          ...
          image: getImg(product?.image_url),
          ...
          brand: product?.brand || "",
          "level 1": product?.level1 || product?.category2 || "Meat",
          "level 2": product?.level2 || product?.category || "",
          "level 3": product?.level3  || "",
          manufacturer_name: product?.manufacturer_name || "",
          distributor_name: product?.distributor_name || " Gordon Food Service",
  ...
  saveCSV(csvString, 'SeaFood8_Data.csv');
  ...
  if (typeof imageUrl === 'string') {
    try {
      ...
      if (Array.isArray(parsedUrls) && parsedUrls.length > 0) {
        return parsedUrls[0]?.url || "";
      }
    } catch (e) {
      return imageUrl; // If parsing fails, return as-is
    }
  }
  return "";
}
