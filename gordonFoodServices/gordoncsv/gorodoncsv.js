...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./json/LambVealID3_Data.json', 'utf8'));
    ...
    const data = jsonArray
      ...
      .map(product => {
        ...
        return {
          ...
          brand: product?.brand || "",
          "level 1": product?.level1 || product?.category1 || "Meat",
          "level 2": product?.level2 || product?.subcategory || "lamb",
  ...
  saveCSV(csvString, 'LambVealID3_Data.csv');
