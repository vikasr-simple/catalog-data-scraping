...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./all_products_details.json', 'utf8'));
  ...
  function jsonToCSV(jsonArray) {
    ...
    const data = jsonArray
      .filter(product => product?.title) // Remove rows without a 'name'
      .map(product => {
        ...
        const mappedProduct = {
          ...
          distributor_name: product?.distrubutor || "Corinthian Distributors",
  ...
  saveCSV(csvString, 'all_products_details.csv');
