import fs from 'fs';
import { parse } from 'json2csv';

try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./Washington_BreakThru.json', 'utf8'));

  function jsonToCSV(jsonArray) {
    // Define the fields for the CSV
    const fields = [
      "description",
      "name",
      "sku",
      "pack",
      "size",
      "gtin",
      "retail_price",
      "is_catch_weight",
      "average_case_weight",
      "image",
      "manufacturer_sku",
      "ordering_unit",
      "is_broken_case",
      "avg_case_weight",
      "content_url",
      "brand",
      "level 1",
      "level 2",
      "level 3",
      "manufacturer_name",
      "distributor_name",
      "unitPrice",
      "extra_data"
    ];

    // Map each JSON object to the desired structure for the CSV
    const data = jsonArray.map(product => ({
      description: product?.description || "",
      name: product?.name || "",
      sku: product?.sku || "",
      pack: product?.pack || "",
      size: product?.size || "",
      gtin: product?.gtin || "",
      retail_price: product?.retail_price || "",
      is_catch_weight: product?.is_catch_weight || "",
      average_case_weight: product?.average_case_weight || "",
      image: getImg(product?.image_url) || "",
      manufacturer_sku: product?.manufacturer_sku || "",
      ordering_unit: product?.ordering_unit || "",
      is_broken_case: product?.is_broken_case || "",
      avg_case_weight: product?.avg_case_weight || "",
      content_url: product?.content_url || "",
      brand: product?.brand || "",
      "level 1": product?.category?.level_1 || "",
      "level 2": product?.category?.level_2 || "",
      "level 3": product?.category?.level_3 || "",
      manufacturer_name: product?.manufacturer_name || "",
      distributor_name: product?.distributor_name || "",
      unitPrice: product?.unitPrice || "",
      extra_data: JSON.stringify(
        Object.keys(product).reduce((acc, key) => {
          if (!fields.includes(key) && key !== 'image_url') {
            acc[key] = product[key];
          }
          return acc;
        }, {})
      ) || ""
      
    }));

    // Convert the array of objects to a CSV string
    return parse(data, { fields });
  }

  // Save the CSV string to a file
  function saveCSV(csvString, filename) {
    fs.writeFileSync(filename, csvString, 'utf8');
    console.log(`CSV file saved as ${filename}`);
  }

  // Convert JSON to CSV and save it
  const csvString = jsonToCSV(jsonData);
  saveCSV(csvString, 'Washington_BreakThru.csv');

} catch (error) {
  console.error("Error processing the file:", error.message);
}

// Helper function to handle image URL
function getImg(imageUrl) {
  if (Array.isArray(imageUrl)) {
    return imageUrl.length > 0 ? imageUrl[0] : "";
  }
  return imageUrl || "";
}
