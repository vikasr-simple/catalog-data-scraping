...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync("./Personal Care.json", "utf8"));
    ...
    const data = jsonArray
      ...
      .map((product) => {
        ...
        const mappedProduct = {
          ...
          sku: product?.sku,
          gtin: product?.gtin,
          image: product?.specifications?.image,
          pack: product?.pack,
          size: product?.size,
          retail_price: product?.specifications?.retail_price,
          ...
          distributor_name: product?.distributor || "Euro Food Mart",
  ...
  saveCSV(csvString, "Personal Care.csv");
} catch (error) {
