...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./thermometers.json', 'utf8'));
    ...
    const data = jsonArray
      ...
      .map(product => {
        ...
        const mappedProduct = {
          ...
          distributor_name: product?.distributor || "King Arthur Baking Company",
  ...
  saveCSV(csvString, 'thermometers.csv');
