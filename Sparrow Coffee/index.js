...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./sparrow.json', 'utf8'));
    ...
    const data = jsonArray
      ...
      .map(product => {
        ...
        const mappedProduct = {
          ...
          sku: product?.sku,
          gtin: product?.gtin,
          image:"https://sparrowcoffee.com" +product?.image,
          pack: product?.pack,
          size: product?.size,
          ...
          distributor_name: product?.distributor || "Sparrow Coffee",
