...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./tsar/tsar.json', 'utf8'));
    ...
    const data = jsonArray
      ...
      .map(product => {
        ...
        const mappedProduct = {
          ...
          sku: product?.sku,
          gtin: product?.gtin,
          image:"https://www.gourmetimports.com/" +product?.image,
          pack: product?.pack,
          size: product?.size,
          ...
          distributor_name: product?.distributor || "Tsar Nicoulai Caviar",
