...
try {
  // Read the JSON file
  const jsonData = JSON.parse(fs.readFileSync('./culinary/truffle-mushroom.json', 'utf8'));
    ...
    const data = jsonArray
      ...
      .map(product => {
        ...
        const mappedProduct = {
          ...
          distributor_name: product?.distributor || "Savory Gourmet",
  ...
  saveCSV(csvString, 'truffle-mushroom.csv');
